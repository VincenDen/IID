...
def main():
    ...
    eval_it_pool = [2000,5000,10000,20000]
        ...
        for it in range(args.Iteration+1):
            ...
            for _ in range(args.outer_loop):
                    ...
                    for image_sign, image_temp in [['syn', image_syn]]:
                        ...
                        for net_ind in range(len(train_net_list)):
                            ...
                            for c in range(num_classes):
                                ...
                                output_syn,output_syn_unflat = embed(img_syn, last=args.embed_last)
                                
                                ###local cov
                                local_real_mean=torch.mean(output_real_unflat,dim=0,keepdim=True)
                                local_real_sub=output_real_unflat-local_real_mean
                                local_real_all=torch.matmul(local_real_sub,local_real_sub.transpose(1,2))
                                local_real_cov=torch.mean(local_real_all,dim=0)

                                local_syn_mean=torch.mean(output_syn_unflat,dim=0,keepdim=True)
                                local_syn_sub=output_syn_unflat-local_syn_mean
                ...
                wandb.log({
                    ...
                    "cov_loss/epoch:{}".format(exp): loss_cov_sum.detach().cpu(),
                    "std_loss/epoch:{}".format(exp): loss_std_sum.detach().cpu(),
                    "epoch":exp})
