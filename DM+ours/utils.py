...
from networks import MLP, ConvNet, LeNet, AlexNet, AlexNetBN, VGG11, VGG11BN, ResNet18, ResNet18BN_AP, ResNet18BN,ResNet18_AP,ResNet_18_my, ViTCIFAR
import  random
...
def get_dataset(dataset, data_path):
    if dataset == 'MNIST':
        ...
        transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        #transform_center = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std),transforms.CenterCrop(30),transforms.Resize(32)])
        dst_train = datasets.CIFAR10(data_path, train=True, download=True, transform=transform) # no augmentation
        #dst_train = datasets.CIFAR10(data_path, train=True, download=True, transform=transform_center)
...
def get_network(model, channel, num_classes, im_size=(32, 32),local=False):
    ...
    if model == 'MLP':
        ...
        net = ConvNet(channel=channel, num_classes=num_classes, net_width=net_width, net_depth=net_depth, net_act=net_act, net_norm=net_norm, net_pooling=net_pooling, im_size=im_size,local=local)
    elif model == 'LeNet':
    ...
    elif model == 'ResNet_18_my':
        net = ResNet_18_my(channel=channel, num_classes=num_classes)
...
def epoch_class(mode, dataloader, net, optimizer, criterion, args, aug):
    loss_avg, acc_avg, num_exp = 0, 0, 0
    acc_avg_class=[0 for i in range(10)]
    num_exp_class=[0 for i in range(10)]
    net = net.to(args.device)
    criterion = criterion.to(args.device)
    ...
    for i_batch, datum in enumerate(dataloader):
        ...
        equal=np.equal(np.argmax(output.cpu().data.numpy(), axis=-1), lab.cpu().data.numpy())
        for j in range(equal.shape[0]):
            if(equal[j]==1):
                acc_avg_class[lab[j]]+=1
            num_exp_class[lab[j]]+=1
        loss_avg += loss.item()*n_b
        acc_avg += acc
        num_exp += n_b
        ...
        if mode == 'train':
            ...
            loss.backward()
            optimizer.step()
    for i in range(len(acc_avg_class)):
        acc_avg_class[i]=acc_avg_class[i]/num_exp_class[i]
    loss_avg /= num_exp
    acc_avg /= num_exp

    return loss_avg, acc_avg,acc_avg_class
...
def evaluate_synset(it_eval, net, images_train, labels_train, testloader, args):
    ...
    # loss_test, acc_test,acc_avg_class = epoch_class('test', testloader, net, optimizer, criterion, args, aug = False)
    print('%s Evaluate_%02d: epoch = %04d train time = %d s train loss = %.6f train acc = %.4f, test acc = %.4f' % (get_time(), it_eval, Epoch, int(time_train), loss_train, acc_train, acc_test))
    return net, acc_train, acc_test
    # return net, acc_train, acc_test,acc_avg_class
...
def get_eval_pool(eval_mode, model, model_eval):
    if eval_mode == 'M': # multiple architectures
        ...
        # model_eval_pool = ['MLP', 'ConvNet', 'LeNet', 'AlexNet', 'VGG11', 'ResNet18',"ViTCIFAR"]
...
## random sample pretrain models
def get_premodel(model_path="/home/dengwenxiao/dc/pre_dict_res18_40"):
    model_list=os.listdir(model_path)
    n=len(model_list)
    idx=random.randint(0, n-1)
    return model_path+'/'+model_list[idx]

def get_other_network(model, channel, num_classes, im_size=(32, 32)):
    torch.random.manual_seed(int(time.time() * 1000) % 100000)
    net_width, net_depth, net_act, net_norm, net_pooling = get_default_convnet_setting()
