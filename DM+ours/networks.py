...
class ConvNet(nn.Module):
    def __init__(self, channel, num_classes, net_width, net_depth, net_act, net_norm, net_pooling, im_size = (32,32),local=False):
    ...
    def embed(self, x):
        out = self.features(x)
        local_feature=out.view(out.size(0), out.size(1),-1)
        out = out.view(out.size(0), -1)

        if(self.local):
            return out,local_feature
        return out
...
def ResNet_18_my(channel, num_classes):
    return ResNet_my(BasicBlock, [2,2,2,2], channel=channel, num_classes=num_classes)
